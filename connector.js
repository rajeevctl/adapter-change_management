 ...
 */
 processRequestResults(error, response, body, callback) {
  ...
  let callbackError = null;
   console.log("In process request Results");
   if (error) {
...
//   };
callOptions.uri=uri;
console.log('calling uri '+callOptions.baseUrl+callOptions.uri);
console.log('UserName  '+callOptions.username+callOptions.password);
  request(callOptions, (error, response, body) => {
    this.processRequestResults(error, response, body, (results, error) => callback(results, error));
  });
}
...
//   getCallOptions.uri = this.constructUri(getCallOptions.serviceNowTable);
   console.log("In Post Method url:");
  ...
  get(callback) {
    ...
    // getCallOptions.uri = `/api/now/table/'+getCallOptions.+'?sysparm_limit=1`;
    console.log("In Get Method url:"+getCallOptions.baseUrl);
    this.sendRequest(getCallOptions, (results, error) => callback(results, error));
  }
